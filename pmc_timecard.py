...
def get_holiday_dates():
    ...
    double_time_holidays = ["New Year's Day", "Memorial Day",
                            "Independence Day (Observed)", "Labor Day", "Veterans Day", "Thanksgiving", "Christmas Day"]
    holiday_datetime = []
...
def new_timecard(filename):
    ...
    save_dir = r'C:/Temp/Timecards/{}/{}'.format(
        role, start_date.strftime("%m-%d-%Y"))
...
def handler(event, context):
    ...
    dbx.files_download_to_file(
        'Activity.csv', '/JHK/Activity/Daily/Activity.csv')

    # copy activity report so we have a history as it gets overwritten everyday
    copyfile('Activity.csv', r'Activity Backups\Activity{}.csv'.format(
    ...
    data = data.sort_values(
        ['Tech', 'Work Order #', 'Status Date'], ascending=(True, True, True))
    df = pandas.DataFrame(columns=['Status', 'Date Created', 'Customer ID', 'Work Order #',
    ...
    for index, row in data.iterrows():
        ...
        current_row_date = row['Status Date'].to_pydatetime(
        ) - timedelta(hours=7)
    ...
    for tech in tech_list:
        ...
        for i, (index, row) in enumerate(tech_data.iterrows()):
            ...
            dv = DataValidation(
                type="list", formula1='"No,Yes"', allow_blank=False)
            time_ws.add_data_validation(dv)
            dv.add(time_ws['K{}'.format(input_row_index)])

            if current_row_event == 'Driving':
                if not driving:
                    time_ws['D{}'.format(input_row_index)
                            ].value = current_row_date
                    driving = True
                ...
                try:
                    ...
                    total_hours = round(
                        total_hours.total_seconds() / 60 / 60, 2)
                    temp_hours = total_hours
                    time_ws['G{}'.format(input_row_index)] = '= SUM(H{}:J{})'.format(
                    ...
                    if current_row_date.date() in dt_holidays:
                        time_ws['J{}'.format(input_row_index)
                                ].value = total_hours
                    elif current_row_date.weekday() == 5 or current_row_date.weekday() == 6:
                        time_ws['I{}'.format(input_row_index)
                                ].value = total_hours
                    elif daily_hours <= 8:
                        time_ws['H{}'.format(input_row_index)
                                ].value = total_hours
                    elif daily_hours > 8:
                        time_ws['I{}'.format(input_row_index)
                                ].value = total_hours
                        if total_hours - (daily_hours - 8) <= 0:
                            ...
                            time_ws['H{}'.format(
                                input_row_index)].value = total_hours - (daily_hours - 8)
                except TypeError as e:
                    print(e)
                input_row_index += 1
            if i == len(tech_data) - 1:
                ...
                try:
                    time_ws['G{}'.format(input_row_index + 1)
                            ] = '= SUM(G3:G{})'.format(input_row_index)
                    time_ws['F{}'.format(input_row_index + 1)
                            ].value = 'Total Hours'
                    ...
                    for col in columns:
                        if col == 'K' or col == 'L':
                            for count in range(3, input_row_index + 32):
                                time_ws['{}{}'.format(col, count)].protection = Protection(
                                    locked=False)
                        else:
